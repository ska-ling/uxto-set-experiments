...
private:
    static constexpr size_t INDEX_NUM_BLOCKS = 1 << 16; // number of hash blocks for level-1 index
    std::vector<size_t> index_block_version_;   // map block -> index version hint
...
public:
    void configure(std::string_view path, bool remove_existing = false) {
        ...
        // Build level-1 index: assign all blocks to current version
        index_block_version_.assign(INDEX_NUM_BLOCKS, current_index_version_);
...
private:
    ...
    std::optional<index_pointer> find_in_index(utxo_key_t const& key, uint32_t height, char op) {
        ...
        // Level-1 hint: compute hash block and get start version
        auto h = key_hash{}(key);
        size_t block = h & (INDEX_NUM_BLOCKS - 1);
        size_t start_v = index_block_version_[block];
        if (start_v > current_index_version_) start_v = current_index_version_;
        // First probe: if hint == current version, search in-memory index
        if (start_v == current_index_version_) {
            if (auto it = index_->find(key); it != index_->end()) {
                search_stats_.add_record(height, 0, 0, false, true, op);
                return it->second;
            }
        } else {
            // probe hinted older version
            try {
                auto [idx_map, cache_hit] = file_cache_.get_or_open_index_file(start_v);
                if (auto it = idx_map.find(key); it != idx_map.end()) {
                    size_t depth = current_index_version_ - start_v;
                    search_stats_.add_record(height, 0, depth, cache_hit, true, op);
                    return it->second;
                }
            } catch (...) {}
        }
        // Fallback: search descending from start_v-1 to 0
        for (size_t v = start_v; v-- > 0;) {
            try {
    ...
    bool insert_in_container(utxo_key_t const& key, span_bytes value, uint32_t height) {
        ...
        try {
            ...
            log_print("Error inserting into index: {}\n", e.what());
            log_print("Rotating index and retrying...\n");
            new_index_version();
        ...
        // update level-1 index hint for this key
        {
            auto h2 = key_hash{}(key);
            size_t blk = h2 & (INDEX_NUM_BLOCKS - 1);
            index_block_version_[blk] = current_index_version_;
        }
