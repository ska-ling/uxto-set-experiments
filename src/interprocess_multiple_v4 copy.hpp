...
public:
    ...
    // Get list of cached files as (container_index, version) pairs
    std::vector<std::pair<size_t, size_t>> get_cached_files() const {
        std::vector<std::pair<size_t, size_t>> cached_files;
        
        // Parse file names to extract container_index and version
        for (auto const& [file_path, cached_file] : cache_) {
            // Expected format: "path/cont_{}_v{:05}.dat"
            // Extract container_index and version from file_path
            auto last_slash = file_path.find_last_of('/');
            std::string filename = (last_slash != std::string::npos) ? 
                file_path.substr(last_slash + 1) : file_path;
            
            // Parse "cont_{container}_v{version}.dat"
            if (filename.starts_with("cont_") && filename.ends_with(".dat")) {
                auto cont_pos = filename.find("cont_") + 5;
                auto v_pos = filename.find("_v", cont_pos);
                auto dat_pos = filename.find(".dat", v_pos);
                
                if (v_pos != std::string::npos && dat_pos != std::string::npos) {
                    try {
                        size_t container_index = std::stoul(filename.substr(cont_pos, v_pos - cont_pos));
                        size_t version = std::stoul(filename.substr(v_pos + 2, dat_pos - v_pos - 2));
                        cached_files.emplace_back(container_index, version);
                    } catch (std::exception const& e) {
                        log_print("file_cache: warning - failed to parse cached file name {}: {}\n", 
                                filename, e.what());
                    }
                }
            }
    ...
    // New get_or_open_file overload taking Index as template parameter and version as normal parameter
    template <size_t Index>
        requires (Index < container_sizes.size())
    std::pair<map_t<Index>&, bool> get_or_open_file(size_t version, std::string_view db_path) {
        auto const file_name = fmt::format(file_format, db_path, Index, version);
        return get_or_open_file<Index>(file_name);
    }
};
...
private:
    ...
    struct deletion_batch {
        ...
        boost::unordered_flat_set<std::string> cached_files;  // Track which files are cached
